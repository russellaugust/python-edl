...
class ParserTestCase(unittest.TestCase):
    ...
    def test_pal(self):
        p = edl.Parser('25')
        path = os.path.join(THIS_DIR, 'test_data/test_25.edl')
        with open(path) as f:
            s = p.parse(f)

    def test_ntsc(self):
        p = edl.Parser('29.97')
        path = os.path.join(THIS_DIR, 'test_data/test_2997NDF.edl')
        with open(path) as f:
            s = p.parse(f)

    def test_2398fps(self):
        p = edl.Parser('23.98')
        path = os.path.join(THIS_DIR, 'test_data/test_2398.edl')
        with open(path) as f:
            s = p.parse(f)
